...
export async function POST(req) {
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (!data.text || data.text.trim().length < 10) {
          results.push({ source: file.name, ok: false, error: "PDF sem texto extraível (pode ser escaneado/imagem)" });
          continue;
        }
    ...
    console.error("Erro no /api/upload:", e);
    return Response.json({ ok: false, error: e.message || "Falha desconhecida no upload" }, { status: 500 });
  }
}
