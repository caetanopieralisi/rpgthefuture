...
export default function Chat() {
  ...
  async function ask() {
    ...
    try {
      ...
      setLog(l => [...l, { role: "ai", text: data.answer || data.error, hadContext: data.hadContext }]);
    } catch {
    ...
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "32px 24px", height: "calc(100vh - 64px)", display: "flex", flexDirection: "column" }}>
        ...
        {log.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{
              ...
              {m.role === "ai" && (
                <div style={{ fontSize: 11, color: "var(--teal)", fontWeight: 600, marginBottom: 8, letterSpacing: 1, display: "flex", justifyContent: "space-between" }}>
                  <span>MESTRE IA</span>
                  {m.hadContext === false && <span style={{ color: "var(--muted)" }}>⚠ sem materiais</span>}
                </div>
              )}
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
        ...
        <button className="btn-primary" onClick={ask} disabled={loading}
          style={{ height: 48, padding: "0 24px", opacity: loading ? 0.6 : 1 }}>
          Enviar
        </button>
      </div>
    </div>
