  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div className="card">
        ...
        {results.map((r,i)=>(
          <div key={i} style={{ marginTop:8, borderRadius:6, fontSize:13, overflow:"hidden" }}>
            <div style={{ padding:"8px 12px",
              background:r.ok?"rgba(77,208,196,0.08)":"rgba(255,107,107,0.08)",
              border:`1px solid ${r.ok?"var(--teal-dim)":"var(--danger)"}`,
              borderRadius: r.warning ? "6px 6px 0 0" : "6px",
              color:r.ok?"var(--teal)":"var(--danger)" }}>
              {r.ok?"✅":"❌"} {r.source} — {r.ok?`${r.indexed}/${r.chunks} blocos indexados`:r.error}
            </div>
            {r.warning && (
              <div style={{ padding:"8px 12px", background:"rgba(240,192,96,0.08)",
                border:"1px solid #a07820", borderTop:"none", borderRadius:"0 0 6px 6px", color:"var(--gold)", fontSize:12 }}>
                ⚠️ {r.warning}
              </div>
            )}
          </div>
