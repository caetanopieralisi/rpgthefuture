...
function FilesTab() {
  ...
  const [jobs, setJobs] = useState([]); // {file, status, indexed, total, done, error}
  ...
  async function indexFile(file) {
    ...
    while (!done) {
      ...
      try {
        ...
        const d = await r.json();
        if (!d.ok) {
          updateJob(file.name, { status: "erro", error: d.error });
          return;
        }
        done = d.done;
        updateJob(file.name, {
          indexed: d.offset,
          total: d.total,
          status: done ? "pronto" : "indexando",
          pct: Math.round((d.offset / d.total) * 100)
        });
      } catch (e) {
        updateJob(file.name, { status: "erro", error: e.message });
        return;
      }
      if (!done) await new Promise(res => setTimeout(res, 800));
    }
  }
  ...
  async function startUpload() {
    ...
    const newJobs = selected.map(f => ({ name: f.name, file: f, status: "aguardando", indexed: 0, total: null, pct: 0 }));
    setJobs(newJobs);
    setSelected([]);

    for (const job of newJobs) {
      updateJob(job.name, { status: "indexando" });
      await indexFile(job.file);
    }
  ...
  async function del(source) {
    ...
    await fetch("/api/files", { method:"DELETE", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ source }) });
    loadFiles();
  }

  const statusColor = s => s === "pronto" ? "var(--teal)" : s === "erro" ? "var(--danger)" : "var(--gold)";
  const statusIcon = s => s === "pronto" ? "✅" : s === "erro" ? "❌" : "⏳";
      ...
      <div className="card">
        ...
        <div onDragOver={e=>{e.preventDefault();setDragOver(true);}}
          ...
          style={{
            ...
            borderRadius:10, padding:28, textAlign:"center", cursor: running?"not-allowed":"pointer",
        ...
        <button className="btn-primary" onClick={startUpload} disabled={running || (!selected.length && !context.trim())}
          style={{ width:"100%", height:44, opacity:running?0.7:1 }}>
          {running ? "Indexando automaticamente..." : "Enviar e indexar"}
        </button>

        {jobs.map((j,i)=>(
          <div key={i} style={{ marginTop:10, padding:"10px 14px", background:"var(--surface)", borderRadius:8, border:"1px solid var(--border)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom: j.status==="indexando"?8:0 }}>
              <span style={{ fontSize:14 }}>{statusIcon(j.status)}</span>
              <span style={{ fontSize:13, flex:1 }}>📄 {j.name}</span>
              <span style={{ fontSize:12, color: statusColor(j.status), fontWeight:600 }}>
                {j.status === "pronto" ? `${j.total} blocos ✓` :
                 j.status === "erro" ? "erro" :
                 j.total ? `${j.indexed}/${j.total} blocos` : "preparando..."}
              </span>
            </div>
            {j.status === "indexando" && j.total && (
              <div style={{ background:"var(--border)", borderRadius:4, height:6 }}>
                <div style={{ width:`${j.pct}%`, background:"var(--teal)", borderRadius:4, height:6, transition:"width 0.5s" }} />
              </div>
            )}
            {j.status === "erro" && <p style={{ fontSize:12, color:"var(--danger)", marginTop:4 }}>{j.error}</p>}
          </div>
        ))}
      </div>
